...
function displayBlocks(blocks) {
    ...
    blocks.forEach((block, index) => {
        ...
        blockElement.innerHTML = `
            <div class="crypto-info">
                <div class="crypto-value">----------->Quokka Block: ${index + 1}:<---------</div>
...
async function displayAmountsByAddress(blocks) {
    ...
    blocks.forEach((block, index) => {
        ...
        const percent = (timestamp / timestampQuokka - 1) * 100000;
...
async function stopMining(csrfToken) {
    ...
    try {
        ...
        if(response.ok) {
			...
			startButton.classList.add('sucess');
			startButton.classList.remove('error');
        } else {
            console.error('Error stopping mining:', response.statusText);
            const startButton = document.getElementById('stopMining');
            startButton.classList.add('success');
            startButton.classList.remove('error'); // Cambiar color a rojo si 
        } 
    } catch (error) {
